 ...
 *
 * This file contains the hal profile for the FELIX FLX-155 (xcvp1552).
 * Derived from the V80 profile. FELIX has no SMBus (out-of-band/BMC path is
 * disabled — the felix CMake profile branch omits -DOOB_ENABLED), so the SMBus
 * HAL below is removed. GCQ mailbox + RPU shared-memory (0x38000000/128M) match
 * felix's DDR remap and are kept unchanged; all XPAR_* come from the felix XSA.
 *
 * @file profile_hal.h
 *
 */
...
/* FELIX has no SMBus IP. The felix profile uses the SMBus FAL *stub*
 * (fw_if_smbus_stub.c, set in CMakeLists) and disables the OOB path, so these
 * are never used at runtime — they exist only so the compiled-but-dormant SMBus
 * FAL still resolves. Do NOT point at XPAR_SMBUS_0 (absent from the felix XSA). */
#define HAL_SMBUS_BASE_ADDR             ( 0x0 )
#define HAL_SMBUS_ADDR                  ( 0x18 )
#define HAL_SMBUS_INTERRUPT             ( 0 )
...
/* FELIX wraps base_logic in static_region/aved, so the GCQ PL-IP XPAR symbol is
 * hierarchically prefixed (V80 had it at top level). Resolves to LPD 0x80000000
 * (gcq_m2r/S01_AXI), matching the traced RPU mailbox address. */
#define HAL_BASE_LOGIC_GCQ_M2R_S01_AXI_BASEADDR ( XPAR_STATIC_REGION_AVED_BASE_LOGIC_GCQ_M2R_BASEADDR )
